...
| `redox-apps/manifest.json` | — | `schedrs` : `git` → `gitlab.com/sory-os/schedrs.git`, `origin` → `fork-sory-os`. `jeremy` : **retiré du manifeste**. `binutils-gdb` : **retiré du manifeste** (échec `all-gdb` : `No rule to make target '../readline/readline/libreadline.a'` ; recette de dev non publiée par redox, `binutils-gdb.pkgar` = 404 sur `static.redox-os.org`). `book` : **retiré du manifeste** (exige `mdbook` host → `command not found` ; `book.pkgar` = 404 sur `static.redox-os.org`) | Le manifeste passe de 305 à **302** recettes. |
